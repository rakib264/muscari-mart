...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    console.warn('[Sitemap] Database connection failed during build, using static routes only:', error instanceof Error ? error.message : String(error));
    isDbConnected = false;
  }
  ...
  if (isDbConnected) {
    try {
      ...
      console.log(`[Sitemap] Generated ${productRoutes.length} product routes, ${categoryRoutes.length} category routes, ${blogRoutes.length} blog routes, ${eventRoutes.length} event routes`);
    } catch (error) {
      console.warn('[Sitemap] Error fetching dynamic routes, using static routes only:', error instanceof Error ? error.message : String(error));
    }
  } else {
